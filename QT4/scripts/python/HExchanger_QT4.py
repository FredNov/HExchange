...
class UIExchange (QtGui.QWidget):
    ...
    def __init__(self):
        ...
        self.coll = QtGui.QCheckBox('Collect with source files:')
        self.coll.setChecked(True)
        self.btn_B_file = QtGui.QToolButton()
        self.btn_B_file.setText('...')
        self.btn_db = QtGui.QToolButton()
        self.btn_db.setText('Dropbox')
        self.nd_view = QtGui.QTreeWidget()
        ...
        self.btn_set = QtGui.QPushButton()
        self.btn_get = QtGui.QPushButton()
        self.btn_upd = QtGui.QPushButton()
    ...
    def setFilePath(self):
        ospath = QtGui.QFileDialog.getExistingDirectory(self, "Select Path to Save", self.ln_path.text())
        if ospath:
    ...
    def updateList(self):      
        ...
        self.nd_view.clear()
        for d in dirs:
            item=QtGui.QTreeWidgetItem(self.nd_view,[os.path.basename(os.path.dirname(d))])
            files=os.listdir(d)
            for f in files:
                QtGui.QTreeWidgetItem(item,[f])
